    ...
    methods: {
      ...
      async toggleEditing() {
        ...
        if (this.toggled.layer && this.state.layer.state.ready && !this.state.loading) {
          ...
          if (!this.state.editing.on && this.state.layer.config.layer_style)  {
            ...
            await CatalogLayersStoresRegistry.getLayerById(this.state.layer.getId()).changeCurrentStyle(this.state.layer.config.layer_style);
          }
