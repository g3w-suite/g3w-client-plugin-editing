    ...
    <div class="editing-layers-features">
        <div v-for="(layerId) in Object.keys($options.layers)">
          ...
          <div class="copy-features-for-layer-content" style="overflow-x: auto">
            <div v-for="(feature, index) in $options.layers[layerId].features" style="padding: 5px; position: relative; display: flex; align-items: baseline; border-bottom: 1px solid #eeeeee;">
              <div style="display: flex; flex-direction: column; border-right: 1px solid #eeeeee">
                ...
                <div v-t-tooltip:right.create="'plugins.editing.steps.help.select_elements'">
                  ...
                  <label :for="`${layerId}_${index}_select_feature_from_layer`" style="color: #FFFFFF"> {{ feature.getId() || 0 }} </label>
                </div>
              </div>
              <div v-for="({attribute, value}) in getAttributesFeature(feature, layerId)" style="display: flex; flex-direction: column; padding: 10px;">
