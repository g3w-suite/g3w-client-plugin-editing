...
module.exports = new (class EditingService extends PluginService {

  constructor() {

    super();

    this.EDITING_FIELDS_TYPE = ['unique']
  ...
  async editResultLayerFeature({
    ...
    try {
      ...
      const features = (_layer.getType() === Layer.LayerTypes.VECTOR) ? source.getFeatures() : source.readFeatures();
      ...
      if (geom) {
        this._mapService.zoomToGeometry(geom);
      }
   ...
   * @FIXME add description
   */
  disableMapControlsConflict(bool=true) {
   ...
   * @private
   */
  _handleOfflineChangesBeforeSave(data) {
   ...
   * @param { Array } setters
   */
  unregisterSettersEvents(setters=[]) {
   ...
   * @private
   */
  _layerChildrenRelationInEditing(layer) {
   ...
   * rollback relations
   */
  rollbackRelations(rollbackItems) {
    Object
   ...
   * @private
   */
  _attachLayerWidgetsEvent(layer) {
   ...
   * @returns {*}
   */
  isFieldRequired(layerId, fieldName) {
   ...
   * @private
   */
  _getToolBoxEditingDependencies(layer) {
   ...
   * @param toolbox
   */
  handleToolboxDependencies(toolbox) {
   ...
   */
  getRelationsAttributesByFeature({
    layerId,
    relation,
    feature,
  } = {}) {
   ...
   */
  getApplicationEditingConstraints() {
   ...
   * @returns {*}
   */
  getApplicationEditingConstraintById(toolboxId) {
   ...
   * @private
   */
  _getRelationFieldsFromRelation({
    layerId,
    relation,
  } = {}) {
    /** {String} @type */
   ...
   */
  createEditingDataOptions(filterType = 'all', options = {}) {
    let filter;
   ...
   */
  async getLayersDependencyFeatures(layerId, opts = {}) {
    const layer     = this.getLayerById(layerId);
   ...
   * @private
   */
  _createCommitMessage(commitItems) {
   ...
   */
  addLayersFeaturesToShowOnResult({
    layerId,
    fids = [],
  }) {
   ...
   */
  clearAllLayersUniqueFieldsValues() {
    this.layersUniqueFieldsValues = {};
  }
   ...
   * @param { string } layerId
   */
  clearLayerUniqueFieldsValues(layerId) {
   ...
   * @param opts.feature
   */
  removeLayerUniqueFieldValuesFromFeature({
    layerId,
    feature,
  }) {
   ...
   * @param opts.feature
   */
  removeRelationLayerUniqueFieldValuesFromFeature({
    layerId,
    relationLayerId,
    feature,
  }) {
   ...
   * @returns { Promise<*> }
   */
  async setLayerUniqueFieldValues(layerId) {
    const promises = [];
    const layer = CatalogLayersStoresRegistry.getLayerById(layerId);
    layer
      .getEditingFields()
   ...
   * @param { string } layerId
   */
  saveTemporaryRelationsUniqueFieldsValues(layerId) {
    const relations = (
      this.layersUniqueFieldsValues[layerId] &&
   ...
   * @param { string } layerId
   */
  clearTemporaryRelationsUniqueFieldsValues(layerId) {
   ...
   * @returns {*}
   */
  getChildLayerUniqueFieldValues({
    layerId,
    relationLayerId,
    field,
  }) {
   ...
   * @returns {Promise<*>}
   */
  async getProjectLayerFeatureById({
    layerId,
    fid,
  }) {
    let feature;
   ...
   * @returns {*}
   */
  getProjectLayersWithSameGeometryOfLayer(layer, options = { exclude: [] }) {
   ...
   */
  getExternalLayersWithSameGeometryOfLayer(layer) {
    const geometryType = layer.getGeometryType();
    return this._mapService
      .getExternalLayers()
   ...
   * @since g3w-client-plugin-editing@v3.7.2
   */
  startEditing(layerId, options = {}, data = false) {
