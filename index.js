  ...
  return Object
    ...
    .map(c =>
      ...
      + `${ commits[c].map(item => {
        ...
        const feat   = features.find(f => id === f.getId());          // original feature
        const efeat  = efeatures.find(f => id === f.getId()) || feat; // edited feature (when deleted fallbacks to original feature)
      ...
      + `</ul><hr>`).join('')
    // edited relations
    + Object.entries(commits.relations || {}).map(r => {
